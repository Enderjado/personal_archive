...
| ------------ | ------ | ----------------------------- |
| `id`         | UUID   | Unique identifier             |
| `value` | String   | "bank", "transaction", "invoice"      |                |
| `type` | Entity   | Topic, date, amount, organization, etc.     |                |
...
| ------------ | ------------ | ---------------------------------------- |
| `documentId` | UUID         | Foreign key to Document                  |
| `vector`     | Array[Float] | Semantic representation of document text |

**Notes:**

* Supports advanced semantic search in the future.
* Stored separately from the main text to optimize storage and query performance.
...
* **Document to Embedding (1:1):** OPTIONAL Each document has a high-dimensional vector representation for search.
